...
def set_data(file, values="zon"):
    ...
    return pd.pivot_table(df, columns="day", index="time", values=values)
...
def plot_monthly_hwm_winds(files):
    ...
    fig, ax = plt.subplots(nrows=3, dpi=300, sharex=True, sharey=True, figsize=(12, 10))
    ...
    for i, file in enumerate(files):
        ...
        ax[i].plot(ds.mean(axis=1), lw=3, color="k", label="Zonal")

        ax[i].set(ylim=[-150, 150], xlim=[ds.index[0], ds.index[-1]])
        ...
        ax[i].plot(ds, color="gray", alpha=0.4)
        ax[i].plot(ds.mean(axis=1), lw=3, color="b", label="Meridional")

        ax[i].axhline(0, linestyle="--")

    ax[0].legend(bbox_to_anchor=(0.5, 1.4), ncol=2, loc="upper center")

    ax[2].set(xlabel="Universal time")

    fig.text(0.03, 0.38, "Velocity (m/s)", fontsize=25, rotation="vertical")
