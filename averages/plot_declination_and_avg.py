...
def set_data(site):
    ...
    path = os.path.join(PATH_PRE, site, years)
    df = b.load(path)
    ...
    return pd.pivot_table(df, values="vp", columns="year", index="month")
...
def plot_declination_difference(ax):
    ...
    for dec_site in [
        ...
        ax_new.plot(doy, dec_site - b.declination(doy), lw=2, linestyle="--")

    ax_new.set(ylim=[-50, 50], xticklabels=[], ylabel="$\\alpha$ (°)")
...
def plot_PRE_monthly():
    ...
    fig, ax = plt.subplots(dpi=300, figsize=(16, 8))
    ...
    ax.set(
        ...
        xticklabels=b.month_names(sort=True, language="pt"),
    )
    fig.text(0.94, 0.46, "$\\alpha$ (°)", rotation="vertical")
    return fig
