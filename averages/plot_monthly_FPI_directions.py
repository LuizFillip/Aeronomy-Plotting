...
def plot_montly_averages(df, ref_date, parameter="tn"):
    ...
    fig, ax = plt.subplots(
        figsize=(16, 8), dpi=300, nrows=2, ncols=2, sharex=True, sharey=True
    )
    ...
    for num, ax in enumerate(ax.flat):
        ...
        ds = pd.pivot(df, columns="day", index="time", values=col)
        ax.plot(
            ds1["time"], ds1[col], color="r", lw=2, label=ref_date.strftime("%Y-%m-%d")
        )
        ax.plot(ds, alpha=0.5, color="gray")
        ax.plot(ds.mean(axis=1), color="k", lw=3)

        ax.set(title=dirs[num].capitalize(), ylim=ylim, yticks=yticks, xlim=[24, 31])
        ax.legend(loc="upper right")
    fontsize = 30

    fig.text(0.05, 0.35, ylabel, fontsize=fontsize, rotation="vertical")
